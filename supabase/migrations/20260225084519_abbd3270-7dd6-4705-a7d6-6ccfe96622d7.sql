...
-- Add related_user_id column to notifications for linking to the user who triggered the notification
ALTER TABLE public.notifications ADD COLUMN related_user_id uuid;

-- Update the interest notification trigger to store the interested user's ID
...
BEGIN
  ...
  -- Create notification with related_user_id
  ...
  VALUES (_service.user_id, 'interest', _notification_title, _notification_message, NEW.service_id, NEW.user_id);
