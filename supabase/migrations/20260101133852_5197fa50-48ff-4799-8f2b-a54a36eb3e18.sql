-- Update get_public_services to show verification status to all users
CREATE OR REPLACE FUNCTION public.get_public_services(_category text DEFAULT NULL::text, _location text DEFAULT NULL::text, _search text DEFAULT NULL::text, _status text DEFAULT 'active'::text)
 RETURNS TABLE(id uuid, user_id uuid, title text, description text, category text, type text, price numeric, price_type text, location text, images text[], status text, accepted_categories text[], created_at timestamp with time zone, updated_at timestamp with time zone, provider_name text, provider_avatar text, provider_linkedin text, provider_facebook text, provider_instagram text, provider_verification_status text, provider_is_founder boolean)
 LANGUAGE plpgsql
...
BEGIN
  ...
  SELECT 
    ...
    p.verification_status AS provider_verification_status,  -- Now visible to all
...
-- Update get_service_by_id to show verification status to all users
